...
const Hero = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      // Initial states
      gsap.set([trustIndicatorsRef.current, headlineRef.current, subheadlineRef.current, ctaRef.current, trustElementsRef.current], { 
        opacity: 0, 
        y: 20 
      });
      ...
      .to(trustElementsRef.current, { 
        opacity: 1, 
        y: 0, 
        duration: 0.8, 
        ease: "power2.out" 
      }, "-=0.4")
      .to([floatingCard1Ref.current, floatingCard2Ref.current], { 
        opacity: 1, 
        y: 0,
        rotation: 0,
        duration: 0.8, 
  ...
  return (
    <section ref={heroRef} className="relative min-h-[90vh] flex items-center justify-center overflow-hidden border-b border-border">
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div className="max-w-4xl mx-auto">
          {/* Trust indicators */}
          <div ref={trustIndicatorsRef} className="flex items-center justify-center space-x-1 mb-6">
          ...
          {/* Main headline */}
          <h1 ref={headlineRef} className="heading-hero text-primary mb-6 transition-colors duration-300 hover:text-primary/80">
            Expert 11+ & SEN Tutoring
            <br />
            <span className="text-foreground transition-colors duration-300 hover:text-primary">by Temitope Adeniyi</span>
          </h1>

          {/* Subheadline */}
          <p ref={subheadlineRef} className="body-large text-foreground/80 mb-8 max-w-2xl mx-auto transition-colors duration-300 hover:text-foreground">
          ...
          <div ref={trustElementsRef} className="mt-12 flex flex-wrap items-center justify-center space-x-8 text-sm text-foreground/70">
            <div className="flex items-center space-x-2 group">
              <div className="w-2 h-2 bg-secondary rounded-full transition-all duration-300 group-hover:bg-secondary/80"></div>
              <span className="transition-colors duration-300 group-hover:text-foreground">20+ Years Experience</span>
            </div>
            <div className="flex items-center space-x-2 group">
              <div className="w-2 h-2 bg-primary rounded-full transition-all duration-300 group-hover:bg-primary/80"></div>
              <span className="transition-colors duration-300 group-hover:text-foreground">SEN Specialist</span>
            </div>
            <div className="flex items-center space-x-2 group">
              <div className="w-2 h-2 bg-accent rounded-full transition-all duration-300 group-hover:bg-accent/80"></div>
              <span className="transition-colors duration-300 group-hover:text-foreground">11+ Expert</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div ref={floatingCard1Ref} className="absolute top-1/4 left-8 bg-card/80 backdrop-blur-sm border border-border p-4 rounded-xl transform rotate-3 hidden lg:block transition-all duration-300 hover:shadow-lg hover:-translate-y-2">
        <div className="text-sm font-semibold text-primary transition-colors duration-300 hover:text-primary/80">
          98%
        </div>
        <div className="text-xs text-muted-foreground">Success Rate</div>
      </div>

      <div ref={floatingCard2Ref} className="absolute bottom-1/4 right-8 bg-card/80 backdrop-blur-sm border border-border p-4 rounded-xl transform -rotate-2 hidden lg:block transition-all duration-300 hover:shadow-lg hover:-translate-y-2">
        <div className="text-sm font-semibold text-secondary transition-colors duration-300 hover:text-secondary/80">
          200+
        </div>
